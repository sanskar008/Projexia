...
const membershipSet = require("../middleware/membershipSet");
...
// Get project by ID (with membership set for fast checks)
router.get("/:id", membershipSet, async (req, res) => {
  try {
...
// Invite a member to a project (with membership set for fast checks)
router.post("/:id/invite", membershipSet, async (req, res) => {
  try {
